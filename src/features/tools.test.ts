...
describe('tools', () => {
  ...
  it('get_league_fixture uses the catalog id for Brazilian Serie A', async () => {
    const { mcp, getHandler } = createMockMcp()
    registerGetLeagueFixtureTool({ mcp } as any)
    const handler = getHandler('get_league_fixture')
    ...
    await handler({ league: 'Brazilian Serie A', week: 1, locale: 'en' })
    expect(fetchMock).toHaveBeenCalled()
    expect(fetchMock.mock.calls[0][0]).toContain('id=71950')
    expect(fetchMock.mock.calls[0][0]).toContain('week=1')
  })
  ...
  it('get_league_fixture uses the catalog id for English Championship', async () => {
    const { mcp, getHandler } = createMockMcp()
    registerGetLeagueFixtureTool({ mcp } as any)
    const handler = getHandler('get_league_fixture')
    ...
    await handler({ league: 'English Championship', week: 1, locale: 'en' })
    expect(fetchMock).toHaveBeenCalled()
    expect(fetchMock.mock.calls[0][0]).toContain('id=70340')
    expect(fetchMock.mock.calls[0][0]).toContain('week=1')
  })

  it('get_league_fixture uses the catalog id for Danish Super League', async () => {
    const { mcp, getHandler } = createMockMcp()
    registerGetLeagueFixtureTool({ mcp } as any)
    const handler = getHandler('get_league_fixture')
    ...
    await handler({ league: 'Danish Super League', week: 1, locale: 'en' })
    expect(fetchMock).toHaveBeenCalled()
    expect(fetchMock.mock.calls[0][0]).toContain('id=70183')
    expect(fetchMock.mock.calls[0][0]).toContain('week=1')
  })
  ...
  it('get_league_fixture uses the catalog id for Scottish Premiership', async () => {
    const { mcp, getHandler } = createMockMcp()
    registerGetLeagueFixtureTool({ mcp } as any)
    const handler = getHandler('get_league_fixture')
    ...
    await handler({ league: 'Scottish Premiership', week: 1, locale: 'en' })
    expect(fetchMock).toHaveBeenCalled()
    expect(fetchMock.mock.calls[0][0]).toContain('id=70290')
    expect(fetchMock.mock.calls[0][0]).toContain('week=1')
  })
