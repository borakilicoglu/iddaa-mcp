...
export function registerGetDetailedEventsTool({ mcp }: McpToolContext): void {
  mcp.tool(
    ...
    async ({ st = 1, type = 0, version = 0, limit = 1000 }) => {
      try {
        const [eventsResponse, competitionsResponse, marketConfigResponse]
          = await Promise.all([
            fetch(
        ...
        if (
          ...
          throw new Error(
            `HTTP error! Events: ${eventsResponse.status}, Competitions: ${competitionsResponse.status}, MarketConfig: ${marketConfigResponse.status}`,
          )
        }
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Error fetching matched events: ${errorMessage}`,
            },
          ],
        }
      }
