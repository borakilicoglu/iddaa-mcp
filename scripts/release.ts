...
function run(command: string, cwd: string) {
  console.warn(`Executing: ${command} in ${cwd}`)
...
function bumpVersion(pkgPath: string, type: 'major' | 'minor' | 'patch' | string, isAlpha: boolean = false): string {
  ...
  console.warn(`Bumped version from ${currentVersion} to ${newVersion} in ${pkgJsonPath}`)
  return newVersion
}
...
function createGitCommitAndTag(version: string, isAlpha: boolean = false) {
  console.warn('Creating git commit and tag...')
  ...
  try {
    ...
    console.warn('Pushing commit and tag to remote...')
    run('git push', rootPath)
    run('git push --tags', rootPath)

    console.warn(`Successfully created and pushed git tag v${version}`)
  }
  catch (error) {
...
async function publishPackage() {
  console.warn(`🚀 Starting ${isAlpha ? 'alpha' : ''} release process...`)
  console.warn(`📝 Version bump: ${versionBumpArg}`)

  // Build package first (assuming a build script exists in package.json)
  console.warn('🔨 Building package...')
  ...
  console.warn(`📤 Publishing package@${newVersion} to npm...`)
  ...
  console.warn(`✅ Successfully completed ${isAlpha ? 'alpha' : ''} release v${newVersion}!`)
}
